...
public class GroupConfigServiceImpl implements IGroupConfigService { 
	...
	*/ 
	...
	public GroupConfigVO getSingleInfo(GroupConfigDO groupConfigDO) {
		...
		groupConfig= iGroupConfigMapper.selectOne(EntityChangeRquestView.createDOToEntity(groupConfigDO,new GroupConfig()));
	...
	*/ 
	...
	public LayuiPage<GroupConfigVO> getGroupConfigListByPage(GroupConfigDO groupConfigDO, LayuiPage<GroupConfigVO> layuiPage){
		  Example example = getPublicExample(groupConfigDO);
		 if(StringUtils.isNotNull(layuiPage.getSort())) {		 	PageHelper.startPage(layuiPage.getPage(), layuiPage.getLimit()).setOrderBy(layuiPage.getSort() + " " + layuiPage.getDire());		 }else {		 	PageHelper.startPage(layuiPage.getPage(), layuiPage.getLimit());		 }		  List<GroupConfigVO> lstVO = new ArrayList<GroupConfigVO>();
		  List<GroupConfig> lst = iGroupConfigMapper.selectByExample(example); 
